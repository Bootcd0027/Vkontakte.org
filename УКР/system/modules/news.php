...
if($logged){
	...
	$limit_news = 20;
			...
			if(!$_POST['page_cnt']){
				...
				$tpl->set('[news]', '');
				$tpl->set('[/news]', '');
				$tpl->set("{buttonsprofileSec-{$type}}", 'buttonsprofileSec');
				$tpl->set('{type}', $type);
			...
			if($sql_){
				...
				foreach($sql_ as $row){
					...
					if($row['action_type'] != 11){
						$rowInfoUser = $db->super_query("SELECT user_search_pref, user_last_visit, user_logged_mobile, user_photo, user_sex, user_privacy, alias FROM `".PREFIX."_users` WHERE user_id = '{$row['ac_user_id']}'");
					...
					if($rowInfoUser['alias']) {
                        ...
                        $tpl->set('{alias}', '/u'.$row['ac_user_id']);
                    }
					} else {
					...
					if($rowInfoUser['adres']) {
                        ...
                        $tpl->set('{alias}', '/u'.$rowInfoUser['ac_user_id']);
                    }
