	...
	switch($act){
		...
		case "login_one":
			...
			if($row['user_balance'] >= $config['nonsense_one_cost'] AND !$check['cnt']){
				...
				//Обновлям кол-во людей и призовой фонд
		...
		case "one":
			...
			if($rowPrizer['user_photo']) $tpl->set('{ava}', '/uploads/users/'.$row['winner_uid'].'/100_'.$rowPrizer['user_photo']);
		...
		case "bilet_buy":
			...
			if($row['user_balance'] >= $config['nonsense_two_cost']){
				...
				//Проверка на то, что нет повторынх чисел
				if($prev_num == 12){
		...
		case "two":
			...
			if($serData >= $tDate AND $row['users_num']){
				...
				//Чистим купленые билеты из кеша
				...
				//Обновляем данные у всех купленых билетов
