	...
	switch($act){
		...
		//Вставляем событие в моментальные оповещания
			...
			if($row_owner['user_last_visit'] >= $update_time){
				$db->query("INSERT INTO `".PREFIX."_updates` SET for_user_id = '{$for_user_id}', from_user_id = '{$user_info['user_id']}', type = '15', date = '{$server_time}', text = '{$action_update_text}', user_photo = '{$user_info['user_photo']}', user_search_pref = '{$user_info['user_search_pref']}', lnk = '/groups?act=requests'");
		...
		case "saveinfo":
			...
			$_POST['city'] = str_replace(array('"', "'"), '', $_POST['city']);
            $city = ajax_utf8(textFilter($_POST['city'], false, true));
			...
			if(stripos($checkAdmin['admin'], "u{$user_id}|") !== false AND isset($title) AND !empty($title) AND $adress_ok){
				...
				if(!$checkAdres['cnt'] AND !$chek_user['cnt'] OR $adres_page == ''){
					$db->query("UPDATE `".PREFIX."_communities` SET title = '".$title."', descr = '".$descr."', comments = '".$comments."', discussion = '{$discussion}', adres = '".$adres_page."', web = '{$web}', web = '{$web}', gtype = '{$gtype}', city = '{$city}' WHERE id = '".$id."'");
					if(!$adres_page)
						echo 'no_new';
				} else
					echo 'err_adres';
		...
		case "all_comm":
			...
			if($row['comments'] OR stripos($row['admin'], "u{$user_id}|") !== false){
				...
				$tpl->set('{user-id}', $public_id);
					...
					else $tpl->set('{alias}', 'u'.$row_comments['public_id']);
				$tpl->set('[comment-form]', '');
				$tpl->set('[/comment-form]', '');
				$tpl->set_block("'\\[record\\](.*?)\\[/record\\]'si","");
				$tpl->set_block("'\\[comment\\](.*?)\\[/comment\\]'si","");
		...
		case"welcom":
			$ids = $_POST['ulist'];
			$groups = intval($_POST['id']);
			$ids= trim($ids,'|');
			$ids = explode('|',$ids);
			$b=0;
			while($ids[$b]){
					$roow = $db->super_query("SELECT * FROM `".PREFIX."_welcom` WHERE uid='{$ids[$b]}' AND myid='{$user_id}' AND groups='{$groups}'");
					if(!$roow){
						$db->query("INSERT INTO `".PREFIX."_welcom` (uid,myid,groups) VALUE ('{$ids[$b]}','{$user_id}','{$groups}')");
						$roows = $db->super_query("SELECT title FROM `".PREFIX."_communities` WHERE id='{$groups}'");
						$title = "Вам отправлено приглашение в группу ".$roows['title'];
						$db->query("INSERT INTO `".PREFIX."_updates` SET for_user_id = '{$ids[$b]}', from_user_id = '{$user_id}', type = '15', date = '{$server_time}', text = 'Вам отправлено приглашение в группу ".$roows['title']."', user_photo = '{$user_info['user_photo']}', user_search_pref = '{$user_info['user_search_pref']}', lnk = '/groups/request'");
						mozg_create_cache("user_{$ids[$b]}/updates", 1);
					}
				$b++;
			}
		break;
		
		//################# Вывод друзей для приглашения #####################
		case"shared":
			if($_GET['page'] > 0) $page = intval($_GET['page']); else $page = 1;
			$gcount = 20;
			$limit_page = ($page-1)*$gcount;
			...
			$ids = intval($_POST['id']);
						...
						if($sql_){
							...
							foreach($sql_ as $row){
								...
								if($user_country_city_name[1])
									...
									$tpl->set('{city}', '');
								$rowwas = $db->super_query("SELECT ulist, del, ban FROM `".PREFIX."_communities` WHERE id = '{$ids}'");
					
								if(stripos($rowwas['ulist'], "|{$row['friend_id']}|") === false AND $rowwas['del'] == 0 AND $rowwas['ban'] == 0){
									$tpl->set('{uservig}', '');
									$tpl->set('{class}', 'grIntiveUser');
								}else{
									$tpl->set('{uservig}', 'в сообществе');
									$tpl->set('{class}', 'grInviteYesed');
								}	
				...
				$tpl->load_template('public/content.tpl');
				echo 1;
			$tpl->set('{grid}', $ids);
			...
			$get_user_id = $user_info['user_id'];
				...
				if(!$CheckBlackList){
					...
					if($friends_sql['user_friends_num']){
						...
						$sql_ = $db->super_query("SELECT SQL_CALC_FOUND_ROWS tb1.friend_id, tb2.user_birthday, user_photo, user_search_pref, user_country_city_name, user_last_visit FROM `".PREFIX."_friends` tb1, `".PREFIX."_users` tb2 WHERE tb1.user_id = '{$get_user_id}' AND tb1.friend_id = tb2.user_id AND tb1.subscriptions = 0 {$sql_order} DESC LIMIT {$num}, 20", 1);
						if($sql_){
							$tpl->load_template('public/friend.tpl');
							foreach($sql_ as $row){
								$user_country_city_name = explode('|', $row['user_country_city_name']);
		...
		case "logins":
			...
			if(stripos($row['ulist'], "|{$user_id}|") === false AND $row['del'] == 0 AND $row['ban'] == 0){
				$ulist = $row['ulist']."|{$user_id}|";
				$db->query("UPDATE `".PREFIX."_communities` SET traf = traf+1, ulist = '{$ulist}' WHERE id = '{$id}'");
				$db->query("UPDATE `".PREFIX."_users` SET user_public_num = user_public_num+1,nums_g=nums_g-1 WHERE user_id = '{$user_id}'");
				$db->query("INSERT INTO `".PREFIX."_friends` SET friend_id = '{$id}', user_id = '{$user_id}', friends_date = NOW(), subscriptions = 2");
				$db->query("DELETE FROM `".PREFIX."_welcom`  WHERE uid='{$user_id}' AND groups='{$id}' ");
		...
		case"unlogin":
			$id = intval($_POST['id']);
			$db->query("DELETE FROM `".PREFIX."_welcom`  WHERE uid='{$user_id}' AND groups='{$id}' ");
				$db->query("UPDATE `".PREFIX."_users` SET  nums_g=nums_g-1 WHERE user_id='{$user_id}'");
			mozg_mass_clear_cache_file("user_{$user_id}/profile_{$user_id}|groups/{$user_id}");
		break;
		
		//################## Вывод всех предложений #####################
		case"request":
			$tpl->load_template('groups/heads.tpl');
			$tpl->compile('info');
			
			$sql_ = $db->super_query("SELECT tb1.uid,myid,groups,yess,tb2.id,photo,adres,title,traf,tb3.user_id,user_search_pref,user_photo 
			FROM `".PREFIX."_welcom`tb1,`".PREFIX."_communities`tb2,`".PREFIX."_users` tb3 
			WHERE tb1.yess='0' AND tb1.uid='{$user_id}' AND tb2.id=tb1.groups AND tb3.user_id=tb1.myid ORDER BY tb1.id DESC LIMIT {$limit_page}, {$gcount}",1);
		
			if($sql_){
			 
				$rows = $db->super_query("SELECT COUNT(*) AS cnt
			FROM `".PREFIX."_welcom`tb1,`".PREFIX."_communities`tb2,`".PREFIX."_users` tb3 
			WHERE tb1.yess='0' AND tb1.uid='{$user_id}' AND tb2.id=tb1.groups AND tb3.user_id=tb1.myid ");
				$tpl->load_template('groups/groupss.tpl');
				foreach($sql_ as $row){
					...
					if($row['user_photo'] != '')
						...
						$tpl->set('{ava}', '/templates/Default/images/50_no_ava.png');
					if($row['adres'] == '')
		...
		default:
			...
			$owner = $db->super_query("SELECT user_public_num,user_ag FROM `".PREFIX."_users` WHERE user_id = '{$user_id}'");
			if($owner['user_ag']<10)$tpl->set('{create_group}', '<a href="/groups" onClick="groups.createbox(); return false"><div><b>Создать сообщество</b></div></a>'); 
            if($owner['user_ag']>=10)$tpl->set('{create_group}', '');
