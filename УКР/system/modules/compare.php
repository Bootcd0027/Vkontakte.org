...
if($logged){
	...
	switch($act){
		...
		case "choose":
				...
				$out = $_GET['out'];
				if($out == 1){
					$tpl->set('{activetab-3}', 'buttonsprofileSec');
				} elseif ($out == 2){
					$tpl->set('{activetab-4}', 'buttonsprofileSec');
				} else {
					$tpl->set('{activetab-2}', 'buttonsprofileSec');
				}
				...
				foreach($likes as $row){
					...
					//Верификация профиля
			        if($row['ureal'] == 1) {
                        $tpl->set('{ureal}', '<div class="search_verified" onmouseover="myhtml.title('. $row['from_user_id'].',\''.'Подтверждённая страница \', \''.'newBBlockl'.'\')" id="newBBlockl'. $row['from_user_id'].'"></div>');
                    } else {
                        $tpl->set('{ureal}', '' );
                    }
