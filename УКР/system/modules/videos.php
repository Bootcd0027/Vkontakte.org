	...
	switch($act){
		...
		case "send":
			...
			if($config['video_mod_add'] == 'yes'){
			    $check = $db->super_query("SELECT time FROM `".PREFIX."_videos` WHERE owner_user_id = '{$user_id}' order by id DESC limit 0,1");
if($check['time'] > (time()-60)) die('ddos');
				...
				if($result_video_lnk AND $title){
					...
					$db->query("INSERT INTO `".PREFIX."_videos` SET owner_user_id = '{$user_id}', video = '{$result_video_lnk}', photo = '{$photo}', title = '{$title}', descr = '{$descr}', add_date = NOW(), original=1, privacy = '{$privacy}', time='{$server_time}' ");
					$dbid = $db->insert_id();
