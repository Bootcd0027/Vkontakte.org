...
switch($act){
	...
	case "view":
		...
		else $tpl->set('{my-ava}', '/templates/Default/images/no_ava_50.png');
	...
	case 'share':
		...
		$wall_text = '<div class="profile_update_photo"><a href="" onClick="pins.view('.$id.'); return false"><img src="/uploads/pins/'.$row['uid'].'/'.$row['sticker'].'" style="margin-top:3px"></a></div>';
	...
	if($sql_){
		...
		if(!$doload || $c_change) $tpl->result['pins'] = '<div class="info_center"><br><br>Ничего не найдено<br><br></div>';
	}
	...
	$tpl->set("{activetab-1}", 'buttonsprofileSec');
