...
if($logged){
	...
	switch($act){
		...
		case "send":
			...
			if($check){
$last_comm = $db->super_query("SELECT * FROM `".PREFIX."_wall` WHERE author_user_id='".$user_info['user_id']."' ORDER BY id DESC LIMIT 1");
$tm = time()-20;
$total_wall = count($db->super_query("SELECT id FROM `".PREFIX."_wall` WHERE author_user_id='".$user_info['user_id']."' AND add_date > '".$tm."' AND add_date < '".time()."'",1));

if(($last_comm['text'] == $wall_text && $last_comm['attach'] == $attach_files) || $total_wall > 2){
echo 'err_flood';
exit;}
				...
				if(isset($wall_text) AND !empty($wall_text) OR isset($attach_files) AND !empty($attach_files)){
					...
					if(!$CheckBlackList){
						if($xPrivasy){
                            ...
                            // Через что опубликовано							
							  if($_SESSION['user_mobile'] == 1) $via = 'через мобильную версию'; else $via == '';
							
													
							//Вставляем саму запись в БД
							$wall_text = tag($wall_text);
							$wall_text = smile ($wall_text);
							$db->query("INSERT INTO `".PREFIX."_wall` SET author_user_id = '{$user_id}', for_user_id = '{$for_user_id}', text = '{$wall_text}', add_date = '{$str_date}', fast_comm_id = '{$fast_comm_id}', via = '".$via."', attach = '".$attach_files."'");
							$dbid = $db->insert_id();
							...
							if($user_id == $for_user_id AND !$fast_comm_id){
								$db->query("INSERT INTO `".PREFIX."_news` SET ac_user_id = '{$user_id}', action_type = 1, action_text = '{$wall_text}', obj_id = '{$dbid}', action_time = '{$str_date}'");
										...
										if($ans_id){ //Если это Ответ кому-либо
			$check_answer = $db->super_query("SELECT author_user_id FROM `".PREFIX."_wall` WHERE id = '{$ans_id}'");
			$d_answer = $db->super_query("SELECT user_privacy, user_last_visit FROM `".PREFIX."_users` WHERE user_id = '{$for_user_id}'");
			   if($check_answer){
			                     //Выводим ид владельца записи					
								
								if($user_id != $check_answer['author_user_id']){
									$db->query("INSERT INTO `".PREFIX."_news` SET ac_user_id = '{$user_id}', action_type = 6, action_text = '{$wall_text}', obj_id = '{$fast_comm_id}', for_user_id = '{$check_answer['author_user_id']}', action_time = '{$str_date}'");
									...
									if($check['user_last_visit'] >= $update_time){
										...
										$db->query("INSERT INTO `".PREFIX."_updates` SET for_user_id = '{$check_answer['author_user_id']}', from_user_id = '{$user_id}', type = '1', date = '{$server_time}', text = '{$wall_text}', user_photo = '{$user_info['user_photo']}', user_search_pref = '{$user_info['user_search_pref']}', lnk = '/wall{$for_user_id}_{$fast_comm_id}'");
									
										mozg_create_cache("user_{$check_answer['author_user_id']}/updates", 1);
									
									//ИНАЧЕ Добавляем +1 юзеру для оповещания
									} else {
										
										$cntCacheNews = mozg_cache('user_'.$check_answer['author_user_id'].'/new_news');
										mozg_create_cache('user_'.$check_answer['author_user_id'].'/new_news', ($cntCacheNews+1));
		...
		include ENGINE_DIR.'/classes/images.php';
			...
			$edit_rev = $db->super_query("SELECT id, author_user_id FROM `".PREFIX."_wall` WHERE id = '{$id}'");
			if($edit_rev['author_user_id'] == $user_id){			
			if($id){
		...
		case "liked_users":
			NoAjaxQuery();
			$rid = intval($_POST['rid']);
			$sql_ = $db->super_query("SELECT tb1.user_id, tb2.user_photo, user_search_pref FROM `".PREFIX."_wall_like` tb1, `".PREFIX."_users` tb2 WHERE tb1.user_id = tb2.user_id AND tb1.rec_id = '{$rid}' ORDER by `date` DESC LIMIT 0, 7", 1);
			if($sql_){
				foreach($sql_ as $row){
					if($row['user_photo']) $ava = '/uploads/users/'.$row['user_id'].'/50_'.$row['user_photo'];
					else $ava = '/templates/'.$config['temp'].'/images/no_ava_50.png';
					echo '<a href="/u'.$row['user_id'].'" id="Xlike_user'.$row['user_id'].'_'.$rid.'" onClick="Page.Go(this.href); return false"><img src="'.$ava.'" width="32" alt="" title="'.$row['user_search_pref'].'" /></a>';
				} 
			}
			die();
		break;
				...
				if(!$id){
					...
					if($row_user){
						...
						if(!$CheckBlackList){
							...
							$tpl->set('{uid}', $id);
							$tpl->set('{rec-id}', $rid);
							$tpl->set("{buttonsprofileSec-{$_GET['type']}}", 'buttonsprofileSec');
							$tpl->compile('info');
