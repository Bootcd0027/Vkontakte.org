	...
	if(empty($hash) or empty($uid) or $hash!=md5($app_id.$uid.$app_secret)) {
        ...
        if($user_info){		// Пускаем, если зарегистрирован
			...
			$md5_pass = $hash;
				...
				$db->query("INSERT INTO `".PREFIX."_users` ( user_vk, user_name, user_lastname, user_photo, user_password, user_group, user_reg_date, user_lastdate, user_privacy, user_search_pref, user_balance, user_email) VALUES ('{$uid}', '{$first_name}', '{$last_name}', '{$photo}', '{$md5_pass}', '5', '{$server_time}', '{$server_time}', 'val_msg|1||val_wall1|1||val_wall2|1||val_wall3|1||val_info|1||val_guests1|1||val_guests2|1||', '{$first_name} {$last_name}', '5', '0')");
