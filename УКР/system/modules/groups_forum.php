...
if($logged){
	...
	switch($act){
		...
		case "prev_msg":
			...
			$sql_ = $db->super_query("SELECT tb1.mid, muser_id, msg, mdate, tb2.user_search_pref, user_photo, user_last_visit, user_logged_mobile, alias FROM `".PREFIX."_communities_forum_msg` tb1, `".PREFIX."_users` tb2 WHERE tb1.muser_id = tb2.user_id AND tb1.fid = '{$id}' AND mid < '{$first_id}' ORDER by `mdate` ASC LIMIT {$start_limit}, {$limit}", 1);
			...
			if($sql_ AND $row2['discussion']){
				...
				foreach($sql_ as $row_comm){
					...
					if($row_comm['alias'])
                        $tpl->set('{alias}', '/'.$row_comm['alias']);
                    else 
                        $tpl->set('{alias}', '/u'.$muser_id['user_id']);
		...
		case "view":
			...
			$row = $db->super_query("SELECT tb1.fid, fixed, title, text, status, fdate, fuser_id, attach, vote, msg_num, public_id, tb2.user_search_pref, user_photo, user_last_visit, user_logged_mobile, alias FROM `".PREFIX."_communities_forum` tb1, `".PREFIX."_users` tb2 WHERE tb1.fid = '{$id}' AND tb1.fuser_id = tb2.user_id");
			
			//Выводим данные о сообществе
			$row2 = $db->super_query("SELECT admin, discussion, adres FROM `".PREFIX."_communities` WHERE id = '{$row['public_id']}'");
			...
			if($row AND $row2['discussion']){
				...
				if($row['msg_num'] > 1){
					...
					$sql_ = $db->super_query("SELECT tb1.mid, muser_id, msg, mdate, tb2.user_search_pref, user_photo, user_last_visit, user_logged_mobile, alias FROM `".PREFIX."_communities_forum_msg` tb1, `".PREFIX."_users` tb2 WHERE tb1.muser_id = tb2.user_id AND tb1.fid = '{$id}' ORDER by `mdate` ASC LIMIT {$sLimit_msg}, {$limit_msg}", 1);
					...
					if($row_comm['alias'])
                        ...
                        $tpl->set('{alias}', '/u'.$row_comm['muser_id']);
				...
				if($row2['adres'])
                        $tpl->set('{alias}', '/'.$row2['adres']);
                    else 
                        $tpl->set('{alias}', '/public'.$public_id);
		...
		default:
			...
			$row = $db->super_query("SELECT forum_num, discussion, ulist, adres FROM `".PREFIX."_communities` WHERE id = '{$public_id}'");
