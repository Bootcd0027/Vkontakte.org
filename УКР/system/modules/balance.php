...
if($logged){
	...
	//################### История by Mario ###################//
		case "history":
			...
			$tpl->compile('info');
		    $sql_ = $db->super_query("SELECT user_id,price,type,date,status FROM `".PREFIX."_history` WHERE user_id = '{$user_id}' ORDER by `date` DESC LIMIT 15",1);
            if($sql_){
				$tpl->load_template('balance/history1.tpl');
				foreach($sql_ as $row){
                    $tpl->set('{price}', $row['price'].' '.gram_record($row['price'], 'price'));					
					if($row['status']=='+')$tpl->set('{status}', '+');
					if($row['status']=='-')$tpl->set('{status}', '-');
					if($row['type']==1)$tpl->set('{type}', '<img src="" / style="width: 32px;height: 32px;background: #DFE4EB url(http://onepuls.ru/templates/Default/images/payments/payments_s.gif) no-repeat 1px 1px;-moz-border-radius: 2px;margin-left: 5px;margin-top: -2px;-webkit-border-radius: 2px;border-radius: 2px;background-position: 1px -135px !important;"><div style="width: 300px;margin-left:50px;margin-top:-25px;">Подарок</div>'); //Подарок от юзера
                    if($row['type']==2)$tpl->set('{type}', '<img src="" / style="width: 32px;height: 32px;background: #DFE4EB url(http://onepuls.ru/templates/Default/images/payments/payments_s.gif) no-repeat 1px 1px;-moz-border-radius: 2px;margin-left: 5px;margin-top: -2px;-webkit-border-radius: 2px;border-radius: 2px;background-position: 1px -167px !important;"><div style="width: 300px;margin-left:50px;margin-top:-25px;">Приглашение друга</div>'); //Приглашение друга
					if($row['type']==3)$tpl->set('{type}', '<img src="" / style="width: 32px;height: 32px;background: #DFE4EB url(http://onepuls.ru/templates/Default/images/payments/payments_s.gif) no-repeat 1px 1px;-moz-border-radius: 2px;margin-left: 5px;margin-top: -2px;-webkit-border-radius: 2px;border-radius: 2px;background-position: 1px -67px !important;"><div style="width: 300px;margin-left:50px;margin-top:-25px;">За посещение</div>');	//Голоса каждый день
					if($row['type']==4)$tpl->set('{type}', '<img src="" / style="width: 32px;height: 32px;background: #DFE4EB url(http://onepuls.ru/templates/Default/images/payments/payments_s.gif) no-repeat 1px 1px;-moz-border-radius: 2px;margin-left: 5px;margin-top: -2px;-webkit-border-radius: 2px;border-radius: 2px;background-position: 1px -67px !important;"><div style="width: 300px;margin-left:50px;margin-top:-25px;">Передача голосов</div>');	//Передача голосов
					if($row['type']==5)$tpl->set('{type}', '<img src="" / style="width: 32px;height: 32px;background: #DFE4EB url(http://onepuls.ru/templates/Default/images/payments/payments_s.gif) no-repeat 1px 1px;-moz-border-radius: 2px;margin-left: 5px;margin-top: -2px;-webkit-border-radius: 2px;border-radius: 2px;background-position: 1px -101px !important;"><div style="width: 300px;margin-left:50px;margin-top:-25px;">Пополнение с терминала</div>'); //Пополнение через терминал    
                    if($row['type']==6)$tpl->set('{type}', '<img src="" / style="width: 32px;height: 32px;background: #DFE4EB url(http://onepuls.ru/templates/Default/images/payments/payments_s.gif) no-repeat 1px 1px;-moz-border-radius: 2px;margin-left: 5px;margin-top: -2px;-webkit-border-radius: 2px;border-radius: 2px;background-position: 1px -px !important;"><div style="width: 300px;margin-left:50px;margin-top:-25px;">Пополнение через телефон</div>'); //Пополнение через телефон
                    if($row['type']==7)$tpl->set('{type}', '<img src="" / style="width: 32px;height: 32px;background: #DFE4EB url(http://onepuls.ru/templates/Default/images/payments/payments_s.gif) no-repeat 1px 1px;-moz-border-radius: 2px;margin-left: 5px;margin-top: -2px;-webkit-border-radius: 2px;border-radius: 2px;background-position: 1px -135px !important;"><div style="width: 300px;margin-left:50px;margin-top:-25px;">Процент с продажи подарка</div>'); //Процент с продажи					
					megaDate($row['date'], 1, 1);
					$tpl->compile('content');
				}
			} else msgbox('', '<br /> <br />Вы не выполняли действия с голосами.<br /> <br /><br />', 'info_2'); 
		break;	
		...
		case "upload":
			...
			$image_rename = $rName; // имя фотографии
				...
				if($price AND $row['cnt'] < $rlimit AND $checkCat['cnt']){
					...
					if(file_exists($upDir.$gift_name.'.jpg')){
						...
						if(file_exists($upDir.$gift_name.'.png')){
							...
							$db->query("INSERT INTO `".PREFIX."_gifts_req` SET user_id = '{$user_id}', gift = '{$gift_name}', price = '{$price}', approve = '1', cat = '{$cat}', balance = '0'");
...
<div>Ваш заработок: &nbsp;<b>{$row['balance']}</b> голосов</div>
<div>Цена: &nbsp;<b>{$row['price']}</b> голосов</div>
...
HTML;
		...
		case "get_transmit":
			...
			if($owner['balance_rub'] >= $config['cost_transmit'] AND $owner['user_balance'] >= $num_mix){
				...
				$db->query("INSERT INTO `".PREFIX."_history` SET user_id = '{$user_id}', type = '4', price='{$nums}', status = '-', date = '{$server_time}'");
$db->query("INSERT INTO `".PREFIX."_history` SET user_id = '{$for_user_id}', type = '4', price='{$nums}', status = '+', date = '{$server_time}'");
