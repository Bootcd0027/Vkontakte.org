		...
		NoAjaxQuery();	
			...
			$names = strtr($names, array(' ' => '%')); 
				...
				if(!$get_user_id)
					...
					//Выводим кол-во албомов из таблицы
					$friends_sql = $db->super_query("SELECT name, photo_num, comm_num FROM `".PREFIX."_albums` WHERE user_id = '{$get_user_id}'");
						
					
					//Все друзья
					if($albums_sql['user_friends_num']){
