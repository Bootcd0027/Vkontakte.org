...
function smile($var)
    {
        $var = str_replace("/^[0-9]+:[X-Z]+$/D",'###', $var);
        $replace = array(
            ':angel:'   => "<img src=".$config['home_url']."/uploads/smiles/1.gif>",
        );
...
function gram_record($num,$type){
  ...
  if($numres == 1){
   ...
   $gram_num_record = 'голос';
	...
	if($type == 'bal'){
		if($numres == 1){
			$gram_num_record = 'пульсик';
		} elseif($numres < 5){
			$gram_num_record = 'пульсики';
		} elseif($numres < 21){
			$gram_num_record = 'пульсиков';
		} elseif($numres == 21){
			$gram_num_record = 'пульсик';
		} elseif($numres > 21){
		    $gram_num_record = 'пульсиков';
...
function OnlineTpl1($time){
	global $tpl, $online_time, $lang;
	if($time >= $online_time)
		return $tpl->set('{online1}', '<span class=\'ipsBadge ipsBadge_green\'>on</span>');
	else
		return $tpl->set('{online1}', '');
}
