...
if($logged){
	...
	if($page_cnt){
		...
		$row = $db->super_query("SELECT id, title, descr, traf, ulist, photo,com_type, date, admin, feedback, comments, real_admin, rec_num, del, ban, adres, audio_num, forum_num, discussion, status_text , cover, cover_pos, gtype, web, city , videos_num, preal FROM `".PREFIX."_communities` WHERE ".$sql_where."");
	...
	if($row['del'] == 1){
		...
		$user_speedbar = stripslashes($row['com_type']);
...
;
		...
		if($row['feedback']){
			...
			foreach($sql_feedbackusers as $row_feedbackusers){
			    ...
			    else $alias = "/u{$row_feedbackusers['fuser_id']}";
				$row_feedbackusers['office'] = stripslashes($row_feedbackusers['office']);
				$feedback_users .= "<div class=\"onesubscription onesubscriptio2n\" id=\"fb{$row_feedbackusers['fuser_id']}\"><a href=\"{$alias}\" onClick=\"Page.Go(this.href); return false\"><img src=\"{$ava}\" alt=\"\" /><div class=\"onesubscriptiontitle\">{$row_feedbackusers['user_search_pref']}</div></a><div class=\"nesubscriptstatus\">{$row_feedbackusers['office']}</div></div>";
			}
		...
		foreach($sql_users as $row_users){
			...
			$users .= "<div class=\"onefriend oneusers\" id=\"subUser{$row_users['user_id']}\"><a href= onClick=\"Page.Go(this.href); return false\"><img src=\"{$ava}\"  style=\"margin-bottom:3px\" /></a><a href= onClick=\"Page.Go(this.href); return false\">{$row_users['user_name']}<br /><span>{$row_users['user_lastname']}</span></a></div>";
		}
		$tpl->set('{users}', $users); 
