...
if($logged){
	...
	if(isset($query) AND !empty($query)){
		...
		if($type == 1) 
			$sql_query = "SELECT user_id, user_search_pref, user_photo, user_birthday, user_country_city_name FROM `".PREFIX."_users` WHERE user_search_pref LIKE '%".$query."%' ORDER by `user_photo` DESC, `user_country_city_name` DESC LIMIT 0, ".$limit_sql;
		 ...
		 //Если критерий поиск "по сообщества"
			...
			$sql_query = "SELECT id, title, photo, traf, adres FROM `".PREFIX."_communities` WHERE title LIKE '%".$query."%' ORDER by `traf` DESC, `photo` DESC LIMIT 0, ".$limit_sql;
		else
			$sql_query = false;
