...
/* 
	...
	Author: Kargin
	Engine: Vii Engine
	Copyright: VdrugOn.Org (с) 2011
	URL: http://vdrugon.org/
	ICQ: 630-248-456
...
if($logged){
	...
	$metatags['title'] = '{translate=lang_965}';
	switch($act){
	    ...
	    $row_main = $db->super_query("SELECT id, rate FROM `".PREFIX."_miss` WHERE user_id = '{$id}'");
      	if($row_main['id']){
		$user_speedbar = '{translate=lang_966}';
		...
		} else {$user_speedbar = '{translate=lang_967}';		msgbox('', '<br><br>{translate=lang_968}<br><br>', 'info_2');
}
	break;
		
		// Топ пользователей
		case "top":
		$user_speedbar = '{translate=lang_969}';
			//Верх
		...
		}else msgbox('', '<br><br>{translate=lang_970}<br><br>', 'info_2');
