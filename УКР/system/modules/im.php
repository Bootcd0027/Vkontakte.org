	...
	//################### Отправка сообщения ###################//
		case "send":
			...
			$attach_files = str_replace('vote|', 'hack|', $attach_files);
				...
				$tm = time()-30;
				$total_msg = count($db->super_query("SELECT id FROM `".PREFIX."_messages` WHERE from_user_id='".$user_id."' AND date > '".$tm."' AND date < '".time()."'",1));
			
			//Код безопасности
			$session_sec_code = $_SESSION['sec_code'];
			$sec_code = $_POST['sec_code'];		
			if($total_msg < 5   OR !empty($sec_code)) {
			...
			if($user_id != $for_user_id AND $for_user_id AND isset($msg) AND !empty($msg) OR isset($attach_files) OR !empty($attach_files)){
				...
				if($row){
					...
					if($xPrivasy){
						...
						if($config['news_mail_8'] == 'yes' AND $user_id != $for_user_id AND $check2['user_last_visit'] <= $update_time){
...
HTML;
		...
		case "history":
			...
			if(!$first_id){
				...
				if($count['all_msg_num'] > $limit_msg)
					$tpl->result['content'] .= '<div class="cursor_pointer" onClick="im.page('.$for_user_id.'); return false" id="wall_all_records" style="width: 420px"><div class="public_wall_all_comm" id="load_wall_all_records" style="margin-left:0px">Показать предыдущие сообщения</div></div><div id="prevMsg"></div>';
		...
		case "upDialogs":
			...
			if($update){
				...
				if($user_info['user_pm_num']){
					$user_pm_num_2 = "<span>+{$user_info['user_pm_num']}</span>";
