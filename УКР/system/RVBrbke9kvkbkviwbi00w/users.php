...
if($se_uid OR $sort OR $se_name OR $se_email OR $ban OR $delet OR $_GET['regdate']){
	...
	if($delet){$where_sql .= "AND user_delet = 1 AND user_delete_type>0 ";$checked_delet = "checked";}
...
$sql_ = $db->super_query("SELECT user_group, user_search_pref, user_id, user_reg_date, user_last_visit, user_email, user_delet, user_ban, user_balance, ureal FROM `".PREFIX."_users`  {$where_sql} ORDER by {$order_sql} LIMIT {$limit_page}, {$gcount}", 1);
...
<div class="fllogall">Верификация:</div>
 <input type="checkbox" name="ureal" style="margin-bottom:10px" {$checked_ureal} />
<div class="mgcler"></div>
...
<div style="background:#fff;float:left;padding:5px;width:100px;text-align:center;margin-left:1px">{$row['ureal']}</div>
...
<select name="mass_type" class="inpu" style="width:260px">
 ...
 <option value="21">Активировать анкеты</option>
 <option value="20">Забанить до смены имени</option>
