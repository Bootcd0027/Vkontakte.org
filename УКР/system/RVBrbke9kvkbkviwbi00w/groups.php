...
if($se_uid OR $sort OR $se_name OR $se_user_id OR $_GET['ban'] OR $_GET['delet'] OR $_GET['preal']){
	...
	if($_GET['preal']){$where_sql .= "AND preal = 1 ";$checked_preal = "checked";}
...
$sql_ = $db->super_query("SELECT tb1.id, title, date, traf, real_admin, del, ban, rec_num, preal, tb2.user_name FROM `".PREFIX."_communities` tb1, `".PREFIX."_users` tb2 WHERE tb1.real_admin = tb2.user_id {$where_sql} ORDER by {$order_sql} LIMIT {$limit_page}, {$gcount}", 1);
...
<form action="Gjfwokvokw4693pvkomwew30tBsjivjOEOKfek.php" method="GET">
...
<div class="fllogall">Верификация:</div>
 <input type="checkbox" name="preal" style="margin-bottom:10px" {$checked_preal} />
<div class="mgcler"></div>
...
<div style="background:#fff;float:left;padding:5px;width:110px;text-align:center;margin-left:1px">{$row['preal']}</div>
...
<div style="background:#f0f0f0;float:left;padding:5px;width:110px;text-align:center;font-weight:bold;margin-top:-5px;margin-left:1px">Верификация</div>
...
<select name="mass_type" class="inpu" style="width:260px">
 ...
 <option value="5">Поставить верификацию сообществу</option>
 <option value="6">Снять верификацию сообществу</option>
