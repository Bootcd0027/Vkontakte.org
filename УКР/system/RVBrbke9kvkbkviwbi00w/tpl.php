...
switch($act){
	...
	case "loadTpl":
		...
		$file_RVBrbke9kvkbkviwbi00wlude = $_POST['tpl'];
		$file_RVBrbke9kvkbkviwbi00wlude = str_replace(array('..', '...', '/../', '//', './', '\..', '\.'), '', $file_RVBrbke9kvkbkviwbi00wlude);		
		$temp_dir = ROOT_DIR.'/templates/'.$temp;
		$content = @file_get_contents($temp_dir.'/'.$file_RVBrbke9kvkbkviwbi00wlude);
		$format_file = strtolower(end(explode('.', $file_RVBrbke9kvkbkviwbi00wlude)));

		if(is_writable($temp_dir.'/'.$file_RVBrbke9kvkbkviwbi00wlude) && in_array($format_file, $allowed_extensions) && file_exists($temp_dir.'/'.$file_RVBrbke9kvkbkviwbi00wlude))
			echo $content;
		else
	...
	case "save":
		...
		$file_open = $temp_dir.'/'.$file_RVBrbke9kvkbkviwbi00wlude;
	...
	default:
		...
		if(is_dir($temp_dir)){
			...
			echo <<<HTML
<script type="text/javascript" src="/system/RVBrbke9kvkbkviwbi00w/js/jquery.js"></script>
<script type="text/javascript">
var temp = {
	loadTpl: function(folder, tpl){
		$('#loading').fadeIn('fast');
		$.post('/Gjfwokvokw4693pvkomwew30tBsjivjOEOKfek.php?mod=tpl&act=loadTpl', {temp: folder, tpl: tpl}, function(data){
			$('#editable').show();
	...
	save: function(){
		...
		tplname = $('#tplname').val();
		$('#loading').fadeIn('fast');
		$.post('/Gjfwokvokw4693pvkomwew30tBsjivjOEOKfek.php?mod=tpl&act=save', {folder: folder, tpl: tplname, content: content}, function(data){
	...
	openFolder: function(template, folder){
		$('#loading').fadeIn('fast');
		$.post('/Gjfwokvokw4693pvkomwew30tBsjivjOEOKfek.php?mod=tpl&act=opneFolder', {template: template, folder: folder}, function(data){
			$('#loading').fadeOut('fast');
