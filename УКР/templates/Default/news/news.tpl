...
<div style="float:left;width:60px"><div class="ava_mini"><a href="{alias}" onClick="Page.Go(this.href); return false"><img src="{ava}" alt="" /></a><div style="position:absolute;margin-left:9px">{online}</div></div></div>
<div style="float:left;width:635px;margin-left:0px">
<div class="wallauthor" ><a href="{alias}" onClick="Page.Go(this.href); return false">{author}<span style="margin:0 3px 0 5px;"><img src="/templates/Default/images/icon/wall_icon.gif" /></span></a> <span class="color777">{action-type-updates}</span></div>
[wall]<div class="wall_tell_all cursor_pointer" onMouseOver="myhtml.title('{rec-id}', 'Отправить в сообщество или другу', 'wall_tell_all_')" onClick="Repost.Box('{rec-id}'[groups], 1[/groups]); return false "id="wall_tell_all_{rec-id}" style="margin-top:-17px"></div>[/wall]
...
[wall]<div class="public_likes_user_block no_display" id="public_likes_user_block{rec-id}" onMouseOver="groups.wall_like_users_five('{rec-id}'[wall-func], 'uPages'[/wall-func])" onMouseOut="groups.wall_like_users_five_hide('{rec-id}')" style="margin-left:490px">
   ...
   <div class="public_like_strelka_news"></div>
  </div>
...
[wall]<div class="wall_tell cursor_pointer " onMouseOver="myhtml.title('{rec-id}', 'Рассказать друзьям', 'wall_tell_')" onClick="[wall-func]wall.tell[/wall-func][groups]groups.wall_tell[/groups]('{rec-id}'); return false" id="wall_tell_{rec-id}"></div>
 <div class="wall_tell_ok no_display " id="wall_ok_tell_{rec-id}"></div>
  </div>
[/wall]
...
<div class="no_display wall_fast_texatrea" id="fast_textarea_{rec-id}">
 <textarea class="wall_inpst fast_form_width wall_fast_text" style="height:33px;color:#000;margin:0px;;width:614px" id="fast_text_{rec-id}"
...
[comment-form]<div class="wall_fast_opened_form" id="fast_form">
 <input type="text" class="wall_inpst fast_form_width wall_fast_input" value="Комментировать..." id="fast_inpt_{rec-id}" onMouseDown="wall.fast_open_textarea('{rec-id}', 2); return false" style="margin:0px;width:614px" />
 <div class="no_display wall_fast_texatrea" id="fast_textarea_{rec-id}">
 <textarea class="wall_inpst fast_form_width wall_fast_text" style="height:33px;color:#000;margin:0px;;width:613px" id="fast_text_{rec-id}"
	onKeyPress="if(event.keyCode == 10 || (event.ctrlKey && event.keyCode == 13))[wall-func]wall.fast_send[/wall-func][groups]groups.wall_send_comm[/groups]('{rec-id}', '{author-id}', 1)"
 ></textarea>
  <div class="button_div fl_l margin_top_5">
   <button id="fast_buts_{rec-id}"
