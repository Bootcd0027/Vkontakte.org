...
$(document).ready(function() {
    ...
    function uploadStart(file) {
		...
		if(cnt > 1)
			$('#status').html($('<p>{translate=lang_39}' + UploadedFiles + ' {translate=lang_41} ' + cnt + '</p>'));
		else
			$('#status').html('{translate=lang_40}');
		...
		try{
			switch(errorCode){
				case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
					$('.uploadButton').css('width', '0px').css('height', '0px').css('overflow', 'hidden');
					$('.uploadbuttbg').show();
					Box.Info('load_photo_er2', lang_dd2f_no, '{translate=lang_42}', 350, 3000);
					setTimeout(function(){
						$('.uploadButton').css('width', '145px').css('height', '11px').css('overflow', 'inherit');
						$('.uploadbuttbg').hide();
					}, 3000);
...
<div class="buttonsprofile albumsbuttonsprofile">
 <a href="/albums/{user-id}" onClick="Page.Go(this.href); return false;">{translate=lang_43}</a>
 <a href="/albums/view/{aid}" onClick="Page.Go(this.href); return false;">{album-name}</a>
 <div class="activetab"><a href="/albums/add/{aid}" onClick="Page.Go(this.href); return false;"><div>{translate=lang_44}</div></a></div>
</div>
<div class="clear"></div>
<div class="load_photo_quote">{translate=lang_45}</div>
<div class="h1" id="l_text" style="display:none;">{translate=lang_46}</div>
<span id="photos"></span>
<div class="clear"></div>
...
<div class="button_div_gray fl_l" style="margin-left:10px;"><button onClick="Page.Go('/albums/view/{aid}'); return false;">{translate=lang_47}</button></div>
</div>
