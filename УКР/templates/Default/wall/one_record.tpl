[record]<div class="wallrecord wall_upage2" id="wall_record_{rec-id}" style="padding-bottom:4px;margin-top:5px">
 ...
 <div class="infowalltext_f clear">
  ...
  <div class="public_likes_user_block no_display" id="public_likes_user_block{rec-id}" onMouseOver="groups.wall_like_users_five('{rec-id}')" onMouseOut="groups.wall_like_users_five_hide('{rec-id}')" style="margin-left:485px">
...
[comment-form]<span id="updateComm{rec-id}wall_comm"></span><div class="wall_fast_opened_form" id="fast_form">
 <input type="text" class="wall_inpst fast_form_width wall_fast_input" value="Комментировать..." id="fast_inpt_{rec-id}" onMouseDown="wall.fast_open_textarea('{rec-id}', 2); update.wall_comm('{rec-id}'); return false" style="margin:0px;width:613px" />
 <div class="no_display wall_fast_texatrea" id="fast_textarea_{rec-id}">
  <textarea class="wall_inpst fast_form_width wall_fast_text" style="height:33px;color:#000;margin:0px;width:613px" id="fast_text_{rec-id}"
  onKeyPress="if(event.keyCode == 10 || (event.ctrlKey && event.keyCode == 13))wall.fast_send('{rec-id}', '{author-id}', 2)"
 ></textarea>
  <div class="button_div fl_l margin_top_5"><button class="btn btn-success btn-mini" onClick="wall.fast_send('{rec-id}', '{author-id}', 2); return false" id="fast_buts_{rec-id}">Отправить Ctrl+Enter</button></div>
