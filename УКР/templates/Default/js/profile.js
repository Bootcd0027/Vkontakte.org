...
form_open: function(){
	...
	form_close: function(){
		opmsg = $('#opmsg').val();
		if(opmsg != 0){
			$('#opinput').val($('#opmsg').val());
		} else {
			$('#opinput').show();
			$('#opniz').hide();
			$('#opinput').val($('#wall_input_text').val());
		}
	},
	send: function(id){
	if($('#opmsg').val() != 0){
	butloading('opbtn', 56, 'disabled');
	var type = $('#optype').val();
		$.post('/index.php?go=opinion&act=send', {id: id, type: type, msg: $('#opmsg').val()}, function(d){
		butloading('opbtn', 56, 'enabled', 'Отправить');
		$('#opblock').html(d);
	});
	} else
			setErrorInputMsg('opmsg');
	},
	del: function(id){
	$('#comment_'+id).hide();	
	$.post('/index.php?go=opinion&act=del', {cid: id});
	}
}
...
var otherbox = {
  ...
  newphoto: function(){
    viiBox.start();
	$.post('/index.php?go=other&act=newphoto', function(d){
	  viiBox.win('newphoto', d);
	});
  },
	 awayphoto: function(){
    viiBox.start();
	$.post('/index.php?go=other&act=awayphoto', function(d){
	  viiBox.win('awayphoto', d);
	});
  },
   ourphoto: function(){
    viiBox.start();
	$.post('/index.php?go=other&act=ourphoto', function(d){
...
addrating: function(r, i, s){
	...
	Show: function(h){
		...
		var id = (h.replace('onepuls.ru', '')).split('_');
...
var friends = {
	...
	checkCode: function(for_id){
	var val_sec_code = $("#val_sec_code2").val();
	...
	$.get('/antibot/sec_code.php?user_code='+val_sec_code, function(data){
		if(data == 'ok'){
			Box.Close('sec_code');
			friends.add(for_id, val_sec_code);
		} else {
			friends.updateCode();
			$('#code_loading').html('<input type="text" id="val_sec_code2" class="inpst" maxlength="6" style="margin-top:10px;width:110px" />');
			$('#val_sec_code').val('');
			$('#val_sec_code').focus();
		}
	});
	},
	add: function(for_id, sec_code, user_name){
		var rndval = new Date().getTime(); 
		if(for_id){
			Page.Loading('start');
			...
			if(user_name)
				...
				name = $('title').text();
			$.post('/index.php?go=friends&act=send_demand', {for_id: for_id, sec_code: sec_code},  function(data){
			if(data == 'sec') {
				Box.Show('sec_code', 280, 'Введите код с картинки:', '<div style="padding:20px;text-align:center"><div class="cursor_pointer" onClick="friends.updateCode(); return false"><div id="sec_code"><img src="/antibot/antibot.php?rndval=' + rndval + '" alt="" title="Показать другой код" width="120" height="50" /></div></div><div id="code_loading"><input type="text" id="val_sec_code2" class="inpst" maxlength="6" style="margin-top:10px;width:110px" /></div></div>', lang_box_canсel, 'Отправить', 'friends.checkCode('+for_id+'); return false;');
			$('#val_sec_code2').focus();
			} else {
				if(data == 'yes_demand')
	...
	addUserList: function(a,h,t){
		if(t == 1) {post_type = 'adduserlist';}
		if(t == 2) {post_type = 'removeuserlist'}
		$.post('/index.php?go=friends&act='+post_type, {id: a, user_id: h}, function(){
			if($('#r_'+a+'_'+h).hasClass('list_noadd')) {
				$('#r_'+a+'_'+h).removeClass('list_noadd');
				$('#r_'+a+'_'+h).addClass('list_yesadd');
				$('#r_'+a+'_'+h).attr('onClick',"friends.addUserList('"+a+"','"+h+"','2')");
			} else {
				$('#r_'+a+'_'+h).removeClass('list_yesadd');
				$('#r_'+a+'_'+h).addClass('list_noadd');
				$('#r_'+a+'_'+h).attr('onClick',"friends.addUserList('"+a+"','"+h+"','1')");
			}
		});
	},
	menuOpen: function(h){
		if(!$('#button_list_'+h).hasClass('add_list_button')) {
			$('#button_list_'+h).addClass('add_list_button');
			$('#list_menu'+h).show();
			$.post('/index.php?go=friends&act=showlist', {id: h}, function(data){
				$('#list_menu'+h).html(data);
			});
		} else {
			$('#button_list_'+h).removeClass('add_list_button');
			$('#list_menu'+h).hide();
		}
	},
	...
	newList: function(id){
		Box.Show('newList', 410, 'Создание нового списка', '<div style="padding:15px;"><div class="videos_text">Название списка:</div><input type="text" class="videos_input" id="namelist" maxlength="65" style="width:360px;"></div>', lang_box_canсel, 'Сохранить', 'friends.postNewList('+id+'); return false');
	},
	delList: function(id){
		Box.Show('delList', 410, 'Удаление списка друзей', '<div style="padding:15px;text-align:center">Вы уверены, что хотите удалить этот список друзей?</div>', lang_box_canсel, 'Удалить список', 'friends.postDelList('+id+'); return false');
	},
	postNewList: function(id) {
		var text = $('#namelist').val();
		$.post('/index.php?go=friends&act=newlist', {text: text}, function(data){
			Box.Close();
			$('#side_panel').append('<a href="/friends/'+id+'/list'+data+'" class="bloglnkactive" onclick="Page.Go(this.href); return false">'+text+'</a>');
			Box.Info('msg_info', 'Успешно', 'Список создан', 300);
		});
	},
	postDelList: function(id) {
		var text = $('#namelist').val();
		$.post('/index.php?go=friends&act=dellist', {id: id}, function(data){
			Box.Close();
			$('#flist'+id).detach();
			Page.Go('/friends');
		});
	...
	goDelte: function(user_id, atype){
		$('#box_loading').show();
		Box.Info('msg_info', lang_friends_del_title, lang_friends_del_text, 300);
...
//FAVE
var fave = {
	add: function(fave_id, type){
		$('#addfave_load').show();
		$.post('/index.php?go=fave&act=add', {fave_id: fave_id, type: type}, function(data){
			if(data == 'no_user')
			...
			$('#addfave_but').attr('onClick', 'fave.delet('+fave_id+','+type+'); return false').attr('href', '/');
			$('#text_add_fave').text(lang_del_fave);
			$('#addfave_load').hide();
		});
	},
	delet: function(fave_id, type){
		$('#addfave_load').show();
		$.post('/index.php?go=fave&act=delet', {fave_id: fave_id, type: type}, function(data){
			$('#addfave_but').attr('onClick', 'fave.add('+fave_id+','+type+'); return false').attr('href', '/');
			$('#text_add_fave').text(lang_add_fave);
			$('#addfave_load').hide();
		});
	},
	del_box: function(fave_id, type){
		Box.Show('del_fave', 410, lang_title_del_photo, '<div style="padding:15px;">Вы уверены, что хотите удалить эту закладку?</div>', lang_box_canсel, lang_box_yes, 'fave.gDelet('+fave_id+','+type+'); return false');
	},
	gDelet: function(fave_id, type){
		if(type<=0 && type>4) Box.Info('msg_info', 'Ошибка', 'Не правильный тип закладки!', 300);
		else {
			$('#box_loading').show();
			$.post('/index.php?go=fave&act=delet', {fave_id: fave_id, type: type}, function(data){
...
var subscriptions = {
	...
	add2: function(for_user_id){
	
		$.post('/index.php?go=subscriptions&act=add', {for_user_id: for_user_id}, function(d){
			
			$('#text_add_subscription'+for_user_id).text('Отписаться');
			$('#lnk_unsubscription'+for_user_id).attr('onClick', 'subscriptions.del2('+for_user_id+'); return false');
		});
	},
	del2: function(del_user_id){
		
		$.post('/index.php?go=subscriptions&act=del', {del_user_id: del_user_id}, function(){
			
			$('#text_add_subscription'+del_user_id).text('Подписаться');
			$('#lnk_unsubscription'+del_user_id).attr('onClick', 'subscriptions.add2('+del_user_id+'); return false');
		});
	},
...
search:function(id, type){
		...
		if(name.length == 0){
			...
			$.post('/index.php?go=videos&act=search',{name:name, id: id, type:type},function(d){
...
go: function(){
  ...
  //if(query != 0 && query != 'Поиск' && query != 'Начните вводить любое слово или имя'){
   if(CheckRequestSearch(location.href) && type == 1){
		...
		if(popular != 0) all_queryeis_popular = '&popular='+popular;
        else all_queryeis_popular = '';
        res_sort_query = all_queryeis_sex+all_queryeis_day+all_queryeis_month+all_queryeis_year+all_queryeis_country+all_queryeis_city+all_queryeis_online+all_queryeis_user_photo+all_queryeis_popular;
   } else
        res_sort_query = '';
...
var myhtml = {
	...
	toggleCheck: function(h){
		if($('#'+h).hasClass('no_display')) $('#'+h).removeClass('no_display');
		else $('#'+h).addClass('no_display');
	},
...
var wall = {
	...
	send_ask: function(){
		wall_text_ask = $('#wall_text_ask').val();
		for_user_id = $('#fid').val();

		if(wall_text_ask != 0){
			butloading('wall_send_ask', 56, 'disabled');
			$.post('/index.php?go=ask&act=send_ask', {wall_text_ask: wall_text_ask, for_user_id: for_user_id}, function(data){
			$('#wall_text_ask').val('');
			butloading('wall_send_ask', 56, 'enabled', lang_box_send);
			Box.Info('msg_ask', lang_dd2f_no, 'Вопрос успешно отправлен!', 300);
...
Box.Page('/index.php?go=graffiti&id='+id+'&act=add', '', '', 627, 'Граффити на стену', lang_box_canсel, 0, 0, 380, 1, 0, 1, 0, 0);
	...
	viiBox.start();
	$.post('/ajax_mapsBox',{id:id}, function(d){
	viiBox.win('maps', d);
	});
},
	tell: function(id){
		...
		$.post('/index.php?go=wall&act=tell', {rid: id}, function(data){
			$('#podelwall'+id).html('Вы рассказали друзьям');
			$('#podelwall'+id).attr('onClick', '');
			if(data == 1)
...
var news = {
	page: function(){
        ...
        gr_list: function(user_id, c, j){
		if(!j) j = 0;
		if(!j) viiBox.start();
		$.post('/index.php?go=gr_list&act=view', {user_id: user_id, section: j}, function(d){
			if(!j) viiBox.win('gr_list', d);
			else {
				var type = $('#type').val();
				$('#tb_box_'+type).hide();
				$('#tb_box_'+j).show().html('<div class="gifts_section clear_fix" id="gr_list">'+d+'</div>');
...
var settings = { 
	box: function(user_id, c, j){
		...
		if(!j) j = 0;
		if(!j) viiBox.start();
		$.post('/index.php?go=settings&act=viewsovet', {section: j}, function(d){
			viiBox.win('gifts_box', d);
			
		});
	},
		box2: function(user_id, c, j){
		if(!j) j = 0;
		if(!j) viiBox.start();
	...
	page_subscribes_groups: function(j,i,f){
		var page_cnt = $('#offset').val();
			...
			$.post('/index.php?go=settings&act=viewsovet'+z, {page_cnt: page_cnt}, function(data){
				$('#offset').val(parseInt($('#offset').val())+1);
				$('#fans_rowsmembers').append(data);
			});
...
audio_off: function() {
                $.post('/index.php?go=settings&act=audio_off', function(data){
                                Box.Info('err', 'Оповещения', 'Звуковые оповещения отключены!', 200, 1500);
                });
        },
audio_on: function() {
                $.post('/index.php?go=settings&act=audio_on', function(data){
                                Box.Info('err', 'Оповещения', 'Звуковые оповещения включены!', 200, 1500);
                });
        },
sms: function(){
    viiBox.start();
...
$('#box_loading').show();
		$.post('/index.php?go=settings&act=addobshenie', {text:text}, function(data){
			if(data == 'now_vip') {
				Box.Info('msg_info', lang_obshenie_title, lang_obshenie_yes_text, 130, 1500);
			} else if(data == 'n_money') {
				Box.Info('msg_info', lang_obshenie_title, lang_obshenie_text, 130, 1500);
			} else {
...
recheck: function(){
		var recheck = $('#recheck').val();
			...
			$.post('/index.php?go=recheck&act=recheck', {recheck: recheck}, function(d){
                                        $('.name_errors').hide();
					$('#ok_recheck').show();
			butloading('saverecheck', '89', 'enabled', 'Пересчитать показатели');
			});
	},
...
activate: function(){
		var activate = $('#activate').val();
			butloading('saveactivate', '89', 'disabled', '');
			$.post('/index.php?go=settings&act=activate', {activate: activate}, function(d){
            if(d == 1){
					$('#ok').show();
                    $('#no_text').hide();
                    $('#no_ok').hide();
			...
			butloading('saveactivate', '89', 'enabled', 'Подтвердить страницу');
			});
	},
savendel: function(del){
				butloading('savendel', 69, 'disabled');
				$.post('/index.php?go=settings&act=delete', {del: del}, function(data){
					$('.name_errors').hide();
					$('#alias_msg').show();
					butloading('savendel', 69, 'enabled', 'Удалить!');
				});
	},	
...
$('body').append('<div id="newbox_miniature"><div class="miniature_box"><div class="miniature_pos" style="width: 540px;padding:15px;"><div class="miniature_title fl_l apps_box_text">История активности</div><a class="cursor_pointer fl_r" onclick="Profile.miniatureClose()">Закрыть</a><div style="border-bottom: 1px solid #DAE1E8;margin-top:20px;"></div><div class="clear"></div>'+body+'</div></div></div>');
	...
	isValidName: function(xname){
		var pattern = new RegExp(/^[a-zA-Zа-я-А-Я-0-9]+$/);
	...
	event: function(event){
		...
		if(oi != 'privacyMenu_msg' && oi != 'privacy_lnk_msg' && oi != 'privacyMenu_wall1' && oi != 'privacy_lnk_wall1' && oi != 'privacyMenu_wall2' && oi != 'privacy_lnk_wall2' && oi != 'privacyMenu_wall3' && oi != 'privacy_lnk_wall3' && oi != 'privacyMenu_info' && oi != 'privacy_lnk_info' && oi != 'privacyMenu_group' && oi != 'privacy_lnk_group' && oi != 'privacyMenu_gifts' && oi != 'privacy_lnk_gifts' && oi != 'privacyMenu_audios' && oi != 'privacy_lnk_audios' && oi != 'privacyMenu_sub' && oi != 'privacy_lnk_sub' && oi != 'privacyMenu_guests1' && oi != 'privacy_lnk_guests1' && oi != 'privacyMenu_guests2' && oi != 'privacy_lnk_guests2' && oi != 'privacyMenu_friend' && oi != 'privacy_lnk_friend' && oi != 'privacyMenu_view' && oi != 'privacy_lnk_view')
			$('#privacyMenu_msg, #privacyMenu_wall1, #privacyMenu_wall2, #privacyMenu_wall3, #privacyMenu_info, #privacyMenu_group, #privacyMenu_gifts, #privacyMenu_audios, #privacyMenu_sub, #privacyMenu_guests1, #privacyMenu_guests2, #privacyMenu_friend').fadeOut(120);
	},
	savePrivacy: function(){
		...
		var val_audios = $('#val_audios').val();
		var val_sub = $('#val_sub').val();
		var val_guests1 = $('#val_guests1').val();
		var val_guests2 = $('#val_guests2').val();
		var val_friend = $('#val_friend').val();
		var val_view = $('#val_view').val();

		butloading('savePrivacy', 55, 'disabled');
		$.post('/index.php?go=settings&act=saveprivacy', {val_msg: val_msg, val_wall1: val_wall1, val_wall2: val_wall2, val_wall3: val_wall3, val_info: val_info, val_group: val_group, val_gifts: val_gifts, val_audios: val_audios, val_sub: val_sub, val_guests1: val_guests1, val_guests2: val_guests2, val_friend: val_friend, val_view: val_view}, function(){
			$('#ok_update').show();
...
var gifts = {
	...
	select: function(gid, fid){
		...
		Box.Close(0, 1);
		viiBox.start();
		viiBox.win('show_box_send', '<div class="miniature_box"><div class="miniature_pos" style="width: 450px;padding:0px;"><a class="fl_r tb_close" onclick="viiBox.clos(\'show_box_send\', 1)">Закрыть</a><div class="tb_title" id="tb_title">Отправка подарка</div><div id="tb_box_1"><div class="box_conetnt" id="box_content_send_gift378" style="height: 438px;border-bottom: 0px solid #D7D7D7;"><center style="margin-bottom: 20px;"><img src="/uploads/gifts/'+gid+'.jpg" style="margin-top:30px"></center><b style="color: #45688E;margin: 0px 40px;">Ваше сообщение</b><textarea id="msgfgift'+gid+'" class="inpst" style="width: 350px;height: 50px;margin: 10px 40px;"></textarea><div class="html_checkbox" id="private_gifts'+gid+'" onclick="myhtml.checkbox(this.id);" style="margin: 0px 40px;color: #777;">Показывать моё имя только получателю</div></div><div class="box_controls_wrap"><div class="box_controls"><table cellspacing="0" cellpadding="0" class="fl_r"><tbody><tr><td><div class="button_blue"><button onclick="gifts.send('+gid+', '+fid+')" id="box_butt_create">Отправить</button></div></td><td><div class="button_gray"><button onclick="viiBox.clos(\'show_box_send\', 1)">Отмена</button></div></td></tr></tbody></table></div></div></div></div></div>');
	},
	send: function(gfid, fid){
...
var clubs = {
	...
	saveinfo: function(id){
		...
		if(pattern.test(adres_page)){
			...
			$.post('/index.php?go=clubs&act=saveinfo', {id: id, title: title, descr: descr, website:website, discussion:discussion, links:links, videos:videos, comments: comments, adres_page: adres_page, val_wall1: val_wall1, val_board: val_board, val_intog: val_intog}, function(d){
				if(d == 'err_adres')
	...
	addadmin: function(id){
		var new_admin_id = $('#new_admin_id').val().replace('http://onepuls.ru/u', '');
...
'</div>';
	...
	wall_all_liked_users: function(rid, page_num, liked_num){
		...
		Box.Page('/index.php?go=clubs&act=all_liked_users', 'rid='+rid+'&liked_num='+liked_num+page, 'all_liked_users_'+rid+page_num, 525, lang_wall_liked_users, lang_msg_close, 0, 0, 345, 1, 1, 1, 0, 1);
	},
	...
	all_people: function(public_id, page_num){
		...
		Box.Page('/index.php?go=clubs&act=all_people', 'public_id='+public_id+'&num='+num+page, 'all_peoples_users_'+public_id+page_num, 525, 'Подписчики', lang_msg_close, 0, 0, 345, 1, 1, 1, 0, 1);
	},
...
var groups = {
	...
	saveinfo: function(id){
		...
		if(pattern.test(adres_page)){
			butloading('pubInfoSave', 55, 'disabled');
			$.post('/index.php?go=groups&act=saveinfo', {id: id, title: title, links: links, descr: descr, comments: comments, adres_page: adres_page, discussion: $('#discussion').val()}, function(d){
				if(d == 'err_adres')
					Box.Info('err', 'Ошибка', 'Такой адрес уже занят', 130, 1500);
				else
...
'</div>';
	...
	del: function(rid, type, container){
  $.post('/index.php?go=likes&act=del', {rid: rid, type: type}, function(){
   $('#'+container+' .likeBut').removeClass('likesMy');
   var count = $('#'+container+' .likesNum').text();
   count = count-(1*1);
   $('#'+container+' .tellSettinghs').attr('onClick', 'Kj.likes.add('+rid+', \''+type+'\', \'like_container_'+rid+'_'+type+'\')');
   $('#'+container+' #my_elem').hide();
   if(count){
    langNumric(container+' .likeTitle', count, lang_06, lang_07, lang_08, lang_06, lang_09, liked+''+count+' ');
    $('#'+container+' .likesNum').html(count);
   }else{
    $('#'+container).attr('onMouseOver', 'Kj.likes.hover('+rid+', this.id, 0)');
    $('#'+container).removeClass('likesHoveredClass');
    $('.likesUsersBl').hide();
    $('#'+container+' .likesNum').html('');
   }
  });
 },
	wall_like_users_five_hide: function(){
...
open: function(uid){
		...
		$.post('/index.php?go=im&act=history', {for_user_id: uid}, function(d){
			...
			$('.im_typograf').html('<div class="no_display" id="im_typograf"><img src="'+template_dir+'/images/typing.gif" /> '+aco[0]+' печатает Вам сообщение..</div>');
	...
	send: function(sec_code){
		var for_user_id = $('#for_user_id').val();
		var my_name = $('#my-name').val();
		var my_ava = $('#my-ava').val();
		var msg_text = $('#msg_text').val();
		var rndval = new Date().getTime(); 
		var attach_files = $('#vaLattach_files').val();
		if(msg_text != 0 || attach_files != 0){
			butloading('sending', 56, 'disabled');
			$.post('/index.php?go=im&act=send', {for_user_id: for_user_id, my_name: my_name, my_ava: my_ava, msg: msg_text, attach_files: attach_files, sec_code:sec_code}, function(data){
			if(data == 'sec') {
				Box.Show('sec_code', 280, 'Введите код с картинки:', '<div style="padding:20px;text-align:center"><div class="cursor_pointer" onClick="im.updateCode(); return false"><div id="sec_code"><img src="/antibot/antibot.php?rndval=' + rndval + '" alt="" title="Показать другой код" width="120" height="50" /></div></div><div id="code_loading"><input type="text" id="val_sec_code2" class="inpst" maxlength="6" style="margin-top:10px;width:110px" /></div></div>', lang_box_canсel, 'Отправить', 'im.checkCode(); return false;');
			$('#val_sec_code2').focus();
			} else {
...
var section = {
	open: function(i){
		$('.section').hide();
		for(is=0;is<=6;is++) $('#actove_section'+is).removeClass('buttonsprofileSec');
		$('#x_'+i).show();
		$('#actove_'+i).addClass('buttonsprofileSec');
	}
}
	...
	init: function(i, hi){
		...
		$("#les10_ex2").draggable({
			...
			drag: function(event, ui){
				...
				if(d >= 0){
					$("#les10_ex2").remove();
					$('#cover_restart').html('<div style="width:870px;height:'+hi+'px;position:relative;top:0px;z-index:1" id="les10_ex2"><img src="'+i+'" width="870" id="cover_img" /></div>');
					$('.cover_addut, .cover_descring').show();
					cover.init(i, hi);
				}
				h = parseInt('-'+(hi-230));
				if(d <= h){
					$("#les10_ex2").remove();
					$('#cover_restart').html('<div style="width:870px;height:'+hi+'px;position:relative;top:'+h+'px;z-index:1" id="les10_ex2"><img src="'+i+'" width="870" id="cover_img" /></div>');
					$('.cover_addut, .cover_descring').show();
					cover.init(i, hi);
				}
			}
...
var Radar = {
  on: function(){
   butloading('RadarOn', 49, 'disabled', '');
 $.post('/index.php?go=radar&act=add', function(d){
  
  if (d!='ok'){
   Box.Info('yes_report', 'Радар', d, 300, 2500);
  }else{
   $('#num').text(parseInt($('#num').text())-10);
   $('#comm_num').text(parseInt($('#comm_num').text())-10);
   $('#RadarOn').hide();
   $('#RadarOff').show();
  }
  butloading('RadarOn', 49, 'enable', 'Включить');
 });
  },
  off: function(){
   butloading('RadarOff', 49, 'disabled', '');
 $.post('/index.php?go=radar&act=del', function(d){
  $('#RadarOff').hide();
  $('#RadarOn').show();

  butloading('RadarOff', 49, 'enable', 'Отключить');
 });
  },
  add:function(){
   butloading('Radarcomm', 49, 'disabled', '');
 id=$('#public_id').val();
 var pattern = new RegExp(/^[0-9]+$/);
 if(pattern.test(id)){ 
  $.post('/index.php?go=radar&act=addcomm',{id:id}, function(d){
   ...
   if (d!='ok'){
    ...
    $('#pokaz_on').show();
    $('#pokaz_off').hide();   
    $('#col_pokaz').text(parseInt($('#col_pokaz').text())+1000);   
    $('#num').text(parseInt($('#num').text())-20);
    $('#comm_num').text(parseInt($('#comm_num').text())-20);
   }
   
   
   butloading('Radarcomm', 49, 'enable', 'Пополнить');
  });
 }
  },
    infocomm:function(id){
  var pattern = new RegExp(/^[0-9]+$/);
  if(pattern.test(id)){
   $.post('/index.php?go=radar&act=infocomm',{id:id}, function(d){
    d = d.split('|');
    if(d){
     if(d[1])
      $('#lgimg').attr('src', '/uploads/groups/'+d[2]+'/100_'+d[1]);
     else
      $('#lgimg').attr('src', template_dir+'/images/100_no_ava.png');
     $('#link').attr('href', 'public'+d[2]);
     $('#title').html(d[3]);
     $('#col_pokaz').html(d[0]);
     if (d[0]>0){
      $('#pokaz_on').show();
...
$('html').css({'overflow':'hidden'});$('body').append('<div id="newbox_miniature"><div class="miniature_box"><div class="miniature_pos" style="width: 560px;"><div class="miniature_title fl_l apps_box_text">Просмотр граффити</div><a class="cursor_pointer fl_r" onclick="Profile.miniatureClose()">Закрыть</a><div style="border-bottom: 1px solid #DAE1E8;margin-top:20px;"></div><div class="clear"></div><img src='+ imgSrc +'></div></div></div>');
}

var ask = {
 add: function(id){
        $.post('/index.php?go=ask&act=add',{id:id},function(d){
            $('html').css({'overflow':'hidden'});$('body').append('<div id="modal_box"><div id="box_gifts" class="box_pos" style="display: block;"><div class="box_bg" style="width:679px;margin-top:159.5px;"><div class="box_title" id="box_title_gifts">Задать вопрос<div class="box_close" onclick="Box.Close(\'gifts\', 0); return false;"></div></div><div class="box_conetnt" id="box_content_gifts" style="height:350px;overflow-y:scroll;"><div class="bg_show" style="width:660px;"></div>'+d+'<style>#box_bottom_left_text{padding-top:6px;float:left}</style><div class="clear"></div></div><div class="bg_show_bottom" style="width:662px;"></div><div class="box_footer"><div class="button_div_gray fl_r"><button onclick="Box.Close(\'gifts\', 0); return false;">Отмена</button></div></div></div></div></div>');
        });
    },
    openFormSend: function(id){
        $('#fast_form_'+id+'').show();
        $('#showFormSend'+id+'').hide();
    },
    hideSendForm: function(id){
        $('#fast_form_'+id+'').hide();
        $('#showFormSend'+id+'').show();
    },
    sendAnswer: function(id){
  answer = $('#fast_text_'+id+'').val();
   
  if(answer != 0){
   $.post('/index.php?go=ask&act=answer_ask', {id: id, answer: answer}, function(data){
   $('#fast_text_'+id+'').val('');
            $('#fast_form_'+id+'').hide();
 ...
 del_ask: function(id){
   $.post('/index.php?go=ask&act=del_ask', {id: id}, function(d){
   $('#info_answer_'+id+'').html('<div class="fl_r" style="color:#777;font-weight:normal;text-align: center;padding: 10px;width: 650px;">Вопрос успешно удален.</div>');
   $('#ask_del_'+id+'').hide();
    });
    }
}
...
friends:function(id){
            $.post('/index.php?go=ajaxfriends',{id:id},function(body){
                    $('html').css({'overflow':'hidden'});$('body').append('<div id="modal_box"><div id="box_subscriptions_1"  style="display: block;"><div class="box_bg" style="width:350px;margin-top:30px;"><div class="ajaxrequest"><div class="ajaxrequest_pos" style="width: 325px"><div class="box_title" >Заявки в друзья</div><div class="clear"></div><div class="box_conetnt">'+body+'</div><div class="box_footer" ><div class="button_div fl_r"><button onclick="Box.Close()">Закрыть</button></div><div class="fl_l" style="font-size:11px;margin-top:5px;"><a href="/friends/requests" onClick="Box.Close(); Page.Go(this.href); return false;">Перейти к списку заявок</a></div></div></div></div></div></div>');
            });
    },
}
var epage = {
	saveInfo: function(id){
		...
		if(title != '') {
			if(adres != '') {
				$.post('/index.php?go=editpage&act=saveGeneralInfo', {id:id, title: title, adres: adres, descr: descr, website: website, day: day,
				month: month, years: years, category: category, pcategory: pcategory, comments: comments, audio: audio, contact: contact, videos: videos, links: links, discussion: discussion}, function(data){
...
var maps = {
	m_box: function(){
		...
		if(title != 0){
				...
				$.post('/ajax_mapsSend', {title: title}, function(){
...
//AJAX вывод времени ожидания
   var pEnum = 0;
 var addbox = {
  times: function(){
    viiBox.start();
	$.post('/index.php?go=support&act=times', function(d){
	  viiBox.win('times', d);
	});
  }
  }
	...
	if(num_mix != 0){
	  ...
	  $.post('/index.php?go=balance&act=get_transmit', {uid: i, num_mix: num_mix}, function(d){
	    if(d == 1){
		  ...
		  Box.Info('transmitok', 'Информация', 'Голоса были успешно переведены.', 270);
		}
