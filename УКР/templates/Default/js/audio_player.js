...
var player = {
  open: function(type, uid, aid){
	$('.staticPlbgTitle').removeClass('mini').css({'padding':'17px 17px 8px'});
    if(!uid) uid = '';
	  ...
	  if(ln){
        ...
        if(tPos == 60){
          $('.staticPlbg').animate({'margin-top': -500});
			$('.staticPlbgTitle').addClass('mini').css({'padding':'13px 12px 7px'});
	$('body').append('<script id="auiossj" type="text/javascript">$( init );function init() {$(".staticPlbgTitle").draggable();}</script>');
        }else
          $('.staticPlbg').animate({'margin-top': 49});
      } else {
        var temp = '<div class="staticPlbg staticPlbgLoadig no_display" style="margin-left:-12px"><div class="staticPlbgTitle"><div class="staticpl_ictop"></div><div align="center" style="margin-left:-10px;margin-bottom:1px"><img src="'+template_dir+'/images/load_player.gif" width="32" height="32" /></div></div></div>';
        $('#audioPlayer').html(temp);
        $('.staticPlbgLoadig').fadeIn(300);
		jQpUserId = uid;
        $.post('/index.php?go=audio_player', {get_user_id: uid, aid: aid}, function(d){
          $('#audioPlayer').html('<div class="staticPlbg" id="staticPlbg" style="margin-left:-12px">'+d+'</div>');
 ...
 close: function(){
  ...
  change_list: function(uid){
	...
	$('#page').html('');
    $('.staticPlbg').addClass('page_audio').css('margin', '3px').css('margin-top', '-2px');
    $('.staticpl_progress_bar').css('width', '470px');
    $('.staticpl_audios').css('width', '744px');
    $('.staticpl_shadow').css('width', '678px');
    $('#jQpSeachVal').css('width', '610px').css('float', 'left');
    $('#jQpLoad').css('margin-left', '606px');
    $('.staticpl_rtitle').css('max-width', '439px');
  ...
  reestablish: function(){
	...
	$('.staticPlbg').removeClass('page_audio').css('margin', '20px').css('margin-top', '49px').css('margin-left','-12px').css('margin-top', '-500px');
